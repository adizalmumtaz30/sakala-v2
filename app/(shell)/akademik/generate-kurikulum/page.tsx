  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5 pb-24">
      ...
      <section className="rounded-2xl border border-border bg-surface p-5 shadow-soft">
        ...
        <button type="button" onClick={generateCandidate} disabled={busy || !activeVersion || activeVersion.verification_status !== "verified" || !classIds.length} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-[linear-gradient(135deg,var(--color-brand)_0%,var(--color-violet)_100%)] px-5 py-3 text-sm font-bold text-white shadow-soft disabled:opacity-50"><RefreshCw className="h-4 w-4" /> Generate Kurikulum</button>
      ...
      {candidate.length > 0 && <section className="space-y-4 rounded-2xl border border-border bg-surface p-5 shadow-soft">
        ...
        {selectedIds.length > 0 && <div className="sticky top-2 z-20 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-brand-600/30 bg-brand-50 p-3 text-sm">
          ...
          {bulkEditing ? (
            <div className="flex items-center gap-2"><input type="number" min="0" step="1" autoFocus value={bulkValue} onChange={(e) => setBulkValue(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") applyBulkTarget(); if (e.key === "Escape") { setBulkEditing(false); setBulkValue(""); } }} placeholder="Target JP" className="w-28 rounded-lg border border-border bg-surface px-3 py-2 font-semibold" aria-label="Target JP untuk semua yang dipilih" /><button onClick={applyBulkTarget} className="rounded-lg bg-[linear-gradient(135deg,var(--color-brand)_0%,var(--color-violet)_100%)] px-3 py-2 font-bold text-white">Terapkan</button><button onClick={() => { setBulkEditing(false); setBulkValue(""); }} className="rounded-lg border border-border bg-surface px-3 py-2">Batal</button></div>
          ) : (
            <div className="flex gap-2"><button onClick={() => setBulkEditing(true)} className="rounded-lg bg-[linear-gradient(135deg,var(--color-brand)_0%,var(--color-violet)_100%)] px-3 py-2 font-bold text-white">Atur Target JP</button><button onClick={() => { setSelectedIds([]); setBulkEditing(false); setBulkValue(""); }} className="rounded-lg border border-border bg-surface px-3 py-2">Batalkan</button></div>
          )}
        </div>}
      ...
      {candidate.length > 0 && !success && <div className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-surface/95 px-4 py-3 shadow-lg backdrop-blur"><div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3">
        ...
        <button type="button" onClick={() => setCompareOpen(true)} disabled={validation.status !== "valid" || committing} className="inline-flex items-center gap-2 rounded-xl bg-[linear-gradient(135deg,var(--color-brand)_0%,var(--color-violet)_100%)] px-5 py-3 text-sm font-bold text-white disabled:opacity-50"><Save className="h-4 w-4" /> {committing ? "Menyinkronkan…" : "Simpan & Sinkronkan"}</button>
      </div></div>}
          ...
          <div role="dialog" aria-modal="true" className="mx-auto mt-[10vh] w-full max-w-lg rounded-2xl border border-border bg-surface p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-5 flex flex-wrap justify-end gap-2">
              ...
              <button
                ...
                className="rounded-xl bg-[linear-gradient(135deg,var(--color-brand)_0%,var(--color-violet)_100%)] px-4 py-2.5 text-sm font-bold text-white"
              >
                Lanjutkan
              </button>
            </div>
