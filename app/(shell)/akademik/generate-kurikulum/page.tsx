...
export default function GenerateKurikulumPage() {
  ...
  async function updateSource() {
    ...
    if (sourceMode === "previous") {
      ...
      setCandidate([]); setShowChanges(true); setMessage("Sumber sebelumnya siap ditinjau. Data resmi belum diubah."); setUpdating(false); return;
    }
    if (!sourceUrl.trim() && !fileName) { setMessage("Masukkan link atau pilih file terlebih dahulu."); setUpdating(false); return; }
    setCandidate([]); setShowChanges(true); setMessage("Sumber baru sudah dipilih dan siap ditinjau. Data resmi belum diubah."); setUpdating(false);
  }
  ...
  function onFileChange(event: ChangeEvent<HTMLInputElement>) { setFileName(event.target.files?.[0]?.name ?? ""); setShowChanges(false); }
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 pb-16">
      ...
      <section className="space-y-5 rounded-2xl border border-border bg-surface p-6 shadow-soft">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold text-ink-900">Sumber &amp; Referensi</h2><p className="mt-1 text-sm leading-6 text-ink-600">Gunakan sumber sebelumnya atau siapkan sumber baru.</p></div><button type="button" onClick={() => void updateSource()} disabled={updating} className="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-5 py-3 text-sm font-semibold text-white disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${updating ? "animate-spin" : ""}`} /> {updating ? "Memperbarui…" : "Update Sumber"}</button></div>
        <div className="grid gap-3 md:grid-cols-2"><button type="button" onClick={() => setSourceMode("previous")} className={`rounded-xl border p-4 text-left ${sourceMode === "previous" ? "border-brand-600 bg-brand-50" : "border-border"}`}><p className="font-semibold text-ink-900">Sumber sebelumnya</p><p className="mt-1 text-sm leading-6 text-ink-600">Gunakan sumber yang sudah pernah dipakai.</p></button><button type="button" onClick={() => setSourceMode("new")} className={`rounded-xl border p-4 text-left ${sourceMode === "new" ? "border-brand-600 bg-brand-50" : "border-border"}`}><p className="font-semibold text-ink-900">Sumber baru</p><p className="mt-1 text-sm leading-6 text-ink-600">Masukkan link atau import file.</p></button></div>
        {sourceMode === "previous" ? <select aria-label="Pilih sumber sebelumnya" value={versionId} onChange={(event) => setVersionId(event.target.value)} className="w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm"><option value="">Pilih sumber sebelumnya</option>{versions.filter((version) => version.verification_status === "verified").map((version) => <option key={version.id} value={version.id}>{version.curriculum_name} · {version.regulation_year ?? "—"}</option>)}</select> : <div className="grid gap-3 md:grid-cols-2"><label className="rounded-xl border border-border p-4"><span className="mb-2 flex items-center gap-2 text-sm font-semibold"><Link2 className="h-4 w-4" /> Masukkan Link</span><input value={sourceUrl} onChange={(event) => setSourceUrl(event.target.value)} placeholder="https://..." inputMode="url" className="w-full rounded-lg border border-border bg-surface px-3 py-2.5 text-sm" /></label><label className="cursor-pointer rounded-xl border border-border p-4"><span className="mb-2 flex items-center gap-2 text-sm font-semibold"><FileUp className="h-4 w-4" /> Import File</span><input type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.txt" onChange={onFileChange} className="w-full text-sm" /><span className="mt-2 block text-xs text-ink-500">{fileName || "Belum ada file dipilih"}</span></label></div>}
        {showChanges && <div className="rounded-xl border border-border bg-surface-muted p-4" role="status"><p className="font-semibold text-ink-900">Sumber siap ditinjau</p><p className="mt-1 text-sm leading-6 text-ink-600">Perubahan belum diterapkan ke data resmi.</p></div>}
      </section>

      <section className="space-y-5 rounded-2xl border border-border bg-surface p-6 shadow-soft"><div><h2 className="text-xl font-bold text-ink-900">Parameter</h2><p className="mt-1 text-sm leading-6 text-ink-600">Pilih parameter yang tersedia. Angka JP dapat diisi manual pada Candidate.</p></div><div className="grid gap-4 md:grid-cols-2"><label><span className="mb-2 block text-sm font-semibold text-ink-900">Jenjang</span><select aria-label="Pilih jenjang" value={level} onChange={(event) => { setLevel(event.target.value); setClassIds([]); }} className="w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm"><option value="">Pilih jenjang</option>{levels.map((value) => <option key={value}>{value}</option>)}</select></label><div><p className="mb-2 text-sm font-semibold text-ink-900">Kelas</p><div className="flex flex-wrap gap-2">{eligibleClasses.map((kelas) => <button key={kelas.id} type="button" onClick={() => toggleClass(kelas.id)} aria-pressed={classIds.includes(kelas.id)} className={`rounded-lg border px-3 py-2 text-sm ${classIds.includes(kelas.id) ? "border-brand-600 bg-brand-50 text-brand-700" : "border-border text-ink-700"}`}>{kelas.tingkat} · {kelas.nama_rombel}</button>)}{!busy && !eligibleClasses.length && <p className="text-sm text-ink-500">Belum ada kelas yang sesuai dengan context aktif.</p>}</div></div></div><button type="button" onClick={generateCandidate} disabled={busy || !activeVersion || activeVersion.verification_status !== "verified" || !level || !classIds.length} className="rounded-xl bg-brand-600 px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">Generate Candidate</button></section>
