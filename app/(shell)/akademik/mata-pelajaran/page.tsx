...
export default function MataPelajaranCurriculumPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 pb-16">
    <header className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-brand-600"><BrainCircuit className="h-4 w-4"/> Curriculum Intelligence</div><h1 className="mt-1 text-2xl font-bold text-ink-900">Mata Pelajaran</h1><p className="mt-1 text-sm text-ink-500">Official regulation → review → selection → Mata Pelajaran + Target JP.</p></div><Link href="/akademik" className="rounded-lg border border-border px-4 py-2 text-sm font-semibold">Kembali</Link></header>
    ...
    <section className="rounded-2xl border border-border bg-surface p-6 shadow-soft space-y-6">
      <div><h2 className="text-lg font-bold text-ink-900">Generate Kurikulum</h2><p className="text-sm text-ink-500">Tidak ada hard-coded regulasi. Generate hanya aktif setelah Curriculum Version terverifikasi.</p></div>
    ...
    <section className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><strong>Integrity gate:</strong> sumber resmi yang saya verifikasi saat ini menunjukkan bahwa JDIH Kemenag memang merupakan sumber authority, tetapi pencarian saat ini belum cukup untuk menetapkan regulasi kurikulum terbaru secara aman. Karena itu database sengaja tidak diisi fakta regulasi. citeturn0search8turn0search11</section>
  </div>;
}
