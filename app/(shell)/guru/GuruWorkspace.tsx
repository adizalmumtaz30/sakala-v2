    ...
    <div className="flex flex-col gap-5">
      ...
      <Card className="p-0">
        {filtered.length === 0 ? <EmptyState title={data.length === 0 ? "Belum ada guru" : "Guru tidak ditemukan"} description={data.length === 0 ? "Tambahkan guru untuk mulai mengatur pembagian mengajar." : "Coba nama guru yang lain."} action={data.length === 0 ? <Button size="sm" onClick={openCreate}><Plus size={14} /> Tambah Guru</Button> : undefined} /> : (
          <ul>{filtered.map((guru) => { const color = teacherColor(guru.kodeGuru || guru.id); return <li key={guru.id} style={{ borderLeft: `3px solid ${color.accent}` }} className="flex items-center gap-3.5 border-b border-border px-5 py-3.5 last:border-0 hover:bg-surface-muted/60">
            ...
            <Link href={`/guru/${guru.id}`} className="min-w-0 flex-1"><p className="truncate text-[13.5px] font-medium text-ink-900">{guru.namaGuru}</p><p className="text-[12px] text-ink-400">{guru.kodeGuru}</p></Link>
            <Badge tone={guru.status === "aktif" ? "success" : "neutral"}>{guru.status === "aktif" ? "Aktif" : "Tidak Aktif"}</Badge>
            <div className="flex items-center gap-1">
              <button onClick={() => router.push(`/guru/${guru.id}/jadwal`)} className="group/act rounded-lg p-0.5 transition-transform hover:scale-105" aria-label="Lihat Jadwal" title="Lihat Jadwal"><IconChip icon={<Eye size={14} />} tone="cyan" size="sm" className="opacity-70 group-hover/act:opacity-100" /></button>
              <button onClick={() => router.push(`/jadwal?viewBy=guru&entityId=${guru.id}&autoAdd=1`)} className="group/act rounded-lg p-0.5 transition-transform hover:scale-105" aria-label="Tambah Jadwal" title="Tambah Jadwal"><IconChip icon={<CalendarPlus size={14} />} tone="brand" size="sm" className="opacity-70 group-hover/act:opacity-100" /></button>
              <button onClick={() => openEdit(guru)} className="group/act rounded-lg p-0.5 transition-transform hover:scale-105" aria-label="Ubah Guru" title="Ubah Guru"><IconChip icon={<Pencil size={14} />} tone="amber" size="sm" className="opacity-70 group-hover/act:opacity-100" /></button>
              <button onClick={() => handleDelete(guru.id)} className="group/act rounded-lg p-0.5 transition-transform hover:scale-105" aria-label="Hapus Guru" title="Hapus Guru"><IconChip icon={<Trash2 size={14} />} tone="rose" size="sm" className="opacity-70 group-hover/act:opacity-100" /></button>
            </div>
          </li>; })}</ul>
        )}
      </Card>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Ubah Guru" : "Tambah Guru"}>
        <form action={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="border-t border-border pt-3.5"><button type="button" onClick={() => setShowMore((v) => !v)} className="flex w-full items-center justify-between text-[12.5px] font-medium text-ink-700">Informasi Tambahan<ChevronDown size={16} className={`text-ink-400 transition-transform duration-200 ${showMore ? "rotate-180" : ""}`} /></button>
      ...
      <ImportModal open={importOpen} onClose={() => setImportOpen(false)} title="Impor Data Guru" description="Unggah data guru dari file XLSX/CSV menggunakan template SAKALA." templateUrl="/guru/import/template" templateFilename="Template_Guru_SAKALA_V2.3.xlsx" onValidate={handleValidateImport} onCommit={handleCommitImport} onImported={() => router.refresh()} />
    </div>
  );
}
