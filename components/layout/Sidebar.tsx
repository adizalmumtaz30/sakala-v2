  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-30 flex flex-col border-r border-border bg-surface" style={{ width: "var(--shell-sidebar-w)" }}>
      <div className="flex items-center gap-3 border-b border-border px-5 py-4">
        ...
        <div className="leading-tight"><p className="text-sm font-bold tracking-tight text-ink-900">SAKALA</p><p className="text-[10px] font-semibold tracking-widest text-ink-400">V2 ENTERPRISE</p></div>
      </div>
            ...
            return (
              <div key={item.key}>
                <Link href={item.href} className={`group relative flex items-center gap-3 rounded-[10px] px-3 py-2 text-[13px] font-medium transition-all duration-150 ${active ? "bg-brand-50 text-brand-700" : "text-ink-700 hover:bg-surface-muted"}`}>
                  ...
                  <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-[8px] ${active ? "bg-brand-100 text-brand-600" : "text-ink-400 group-hover:text-ink-700"}`}><Icon size={18} strokeWidth={1.8} /></span>
                  {item.label}
                </Link>
                {item.key === "data" && inDataCore && <div className="ml-[26px] mt-1 flex flex-col gap-0.5 border-l border-border pl-3">{dataSubnav.map(sub => <Link key={sub.href} href={sub.href} className={`rounded-md px-2.5 py-1.5 text-[12px] font-medium ${pathname.startsWith(sub.href) ? "text-brand-700" : "text-ink-500 hover:text-ink-900"}`}>{sub.label}</Link>)}</div>}
                {item.key === "akademik" && inAcademic && <div className="ml-[26px] mt-1 flex flex-col gap-0.5 border-l border-border pl-3">{academicSubnav.map(sub => <Link key={sub.href} href={sub.href} className={`rounded-md px-2.5 py-1.5 text-[12px] font-medium ${pathname.startsWith(sub.href) ? "text-brand-700" : "text-ink-500 hover:text-ink-900"}`}>{sub.label}</Link>)}</div>}
              </div>
            );
          })}
        </div>
