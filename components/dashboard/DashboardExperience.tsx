...
function SmartInsight({ jpInsight, bebanTertinggi, bebanDistribution, scheduleConflicts, curriculumStatus }: { jpInsight: DashboardJpInsight; bebanTertinggi: DashboardWorkloadFullEntry[]; bebanDistribution: DashboardBebanDistribution; scheduleConflicts: DashboardScheduleConflictSummary; curriculumStatus: DashboardCurriculumStatus }) {
  type Signal = { level: "critical" | "warning" | "good"; text: string; href: string };
  const signals: Signal[] = [];
  // §7/§9: kurikulum adalah root cause fondasional — dicek & ditampilkan lebih
  // dulu daripada gejala turunannya (Target JP kosong, dst), supaya operator
  // diarahkan ke akar masalah, bukan ke gejalanya.
  if (!curriculumStatus.curriculumAvailable) {
    signals.push({ level: "warning", text: "Kurikulum resmi untuk konteks ini belum tersedia.", href: "/akademik" });
  } else if (curriculumStatus.unmatchedSubjectCount > 0) {
    signals.push({ level: "warning", text: `${curriculumStatus.unmatchedSubjectCount} mata pelajaran dari kurikulum resmi belum terhubung dengan data Mata Pelajaran.`, href: "/mata-pelajaran" });
  }
...
export default function DashboardExperience({ schoolName, adminName, context, metrics, metricTrends, jpInsight, scheduleConflicts, curriculumStatus, workload, heatmap, heatmapGrid, rooms, heatmapGridByRoom, bebanDistribution, workloadFull, agenda, activity, guruList, notifications }: { schoolName: string; adminName: string | null; context: string | null; metrics: DashboardKeyMetrics; metricTrends: DashboardMetricTrends | null; jpInsight: DashboardJpInsight; scheduleConflicts: DashboardScheduleConflictSummary; curriculumStatus: DashboardCurriculumStatus; workload: DashboardWorkloadEntry[]; heatmap: DashboardHeatmapDay[]; heatmapGrid: DashboardHeatmapGridDay[]; rooms: DashboardRoomLite[]; heatmapGridByRoom: Record<string, DashboardHeatmapGridDay[]>; bebanDistribution: DashboardBebanDistribution; workloadFull: DashboardWorkloadFullEntry[]; agenda: DashboardAgendaEntry[]; activity: DashboardActivityEntry[]; guruList: GuruLite[]; notifications: NotificationEntry[] }) {
