...
function Avatar({ name, size = "md" }: { name: string; size?: AvatarSize }) {
...
function LineChart({ days }: { days: DashboardHeatmapDay[] }) {
  ...
  const points = days.map((d, i) => {
    const x = days.length <= 1 ? width / 2 : pad + (i * (width - pad * 2)) / (days.length - 1);
  ...
  return <div className="relative h-[164px] w-full overflow-hidden rounded-xl bg-surface-muted/45 px-1 pt-1">
    ...
    <div className="absolute inset-x-4 bottom-1 flex justify-between text-[9px] font-medium text-ink-400">{days.map((d) => <Link key={d.day} href={`/analitik?day=${encodeURIComponent(d.day)}`} className="hover:text-brand-600">{d.label.slice(0, 3)}</Link>)}</div>
    <div className="absolute inset-0 grid grid-cols-6">{days.map((d) => <Link key={d.day} href={`/analitik?day=${encodeURIComponent(d.day)}`} aria-label={`Buka analitik ${d.label}`} className="rounded-xl focus-visible:ring-2 focus-visible:ring-brand-500/40`} />)}</div>
  </div>;
}
...
function Donut({ insight }: { insight: DashboardJpInsight }) {
  ...
  return <div className="flex items-center gap-5">
    <Link href="/pembagian-mengajar/target-jp?status=penuh" aria-label={`Target JP ${insight.completionPercent}% terpenuhi`} className="relative h-28 w-28 shrink-0 rounded-full p-[11px] transition-transform hover:scale-[1.02] focus-visible:ring-2 focus-visible:ring-brand-500/40" style={{ background: bg }}>
      <span className="flex h-full w-full items-center justify-center rounded-full bg-surface text-center"><span><strong className="block text-[23px] leading-none tabular-nums text-ink-900">{insight.completionPercent}%</strong><small className="mt-1 block text-[8px] text-ink-400">terpenuhi</small></span></span>
    </Link>
    <div className="min-w-0 flex-1 space-y-2 text-[10px]">
      <Link href="/pembagian-mengajar/target-jp?status=penuh" className="flex justify-between gap-3 hover:text-brand-600"><span>Terpenuhi</span><b className="tabular-nums">{done}</b></Link>
      <Link href="/pembagian-mengajar/target-jp?status=sebagian" className="flex justify-between gap-3 hover:text-brand-600"><span>Belum lengkap</span><b className="tabular-nums">{partial}</b></Link>
      <Link href="/pembagian-mengajar/target-jp?status=kosong" className="flex justify-between gap-3 hover:text-brand-600"><span>Belum mulai</span><b className="tabular-nums">{empty}</b></Link>
    </div>
  </div>;
}

function Metric({ label, value, href }: { label: string; value: number; href: string }) {
  return <Link href={href} className="group min-w-[58px] border-l border-border/70 pl-3"><span className="block text-[17px] font-bold leading-none tabular-nums text-ink-900 group-hover:text-brand-600">{value}</span><span className="mt-1 block text-[9px] text-ink-400">{label}</span></Link>;
}
