...
const BEBAN_STYLE: Record<"ringan" | "normal" | "berat", { label: string; badge: string; dot: string }> = {
  ...
  normal: { label: "Normal", badge: "border-brand-600/20 bg-brand-50 text-brand-700", dot: "bg-brand-600" },
...
function RekapJtm({ heatmap }: { heatmap: DashboardHeatmapDay[] }) {
  const [mode, setMode] = useState<JtmMode>("hari");
...
function BebanDonut({ distribution }: { distribution: DashboardBebanDistribution }) {
  ...
  const bg = `conic-gradient(#10b981 0deg ${a}deg, var(--brand-600) ${a}deg ${b}deg, #f43f5e ${b}deg 360deg)`;
  ...
  return <div className="flex items-center gap-5">
    ...
    <div className="min-w-0 flex-1 space-y-2 text-[10px]">
      ...
      <Link href="/guru" className="flex items-center justify-between gap-3 rounded px-1 hover:bg-brand-50 hover:text-brand-600 focus-visible:ring-2 focus-visible:ring-brand-500/40"><span className="flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-brand-600" />Normal <span className="text-ink-400">(21–32 JP)</span></span><span className="flex items-baseline gap-1 tabular-nums"><b>{distribution.normal}</b><span className="text-[8.5px] text-ink-400">({pct(distribution.normal)}%)</span></span></Link>
...
function HeatmapGrid({ grid }: { grid: DashboardHeatmapGridDay[] }) {
  ...
  return <div className="overflow-x-auto">
    ...
    <p className="mt-2 text-[9px] text-ink-400">Arahkan kursor ke sel untuk melihat detail jam &amp; jumlah jadwal.</p>
  </div>;
}
...
export default function DashboardExperience({ schoolName, context, metrics, jpInsight, workload, heatmap, heatmapGrid, bebanDistribution, workloadFull, agenda, activity, guruList, notifications }: { schoolName: string; context: string | null; metrics: DashboardKeyMetrics; jpInsight: DashboardJpInsight; workload: DashboardWorkloadEntry[]; heatmap: DashboardHeatmapDay[]; heatmapGrid: DashboardHeatmapGridDay[]; bebanDistribution: DashboardBebanDistribution; workloadFull: DashboardWorkloadFullEntry[]; agenda: DashboardAgendaEntry[]; activity: DashboardActivityEntry[]; guruList: GuruLite[]; notifications: NotificationEntry[] }) {
  ...
  return <main className="mx-auto flex min-h-[calc(100vh-5.5rem)] max-w-[1440px] flex-col gap-3 px-2 pb-4 pt-3 sm:px-3 lg:gap-3.5">
    ...
    <div className="grid min-h-0 gap-3 lg:grid-cols-[1.5fr_.72fr]"><Section title="Rekap JTM" description="Jam Tatap Muka committed · klik titik/area untuk membuka analitik." href="/analitik" icon={<Activity size={14} />}><RekapJtm heatmap={heatmap} /></Section><Section title="Distribusi Beban Guru" description="Ringan/Normal/Berat berdasarkan JP committed." href="/guru" icon={<Users size={14} />}><BebanDonut distribution={bebanDistribution} /></Section></div>
    <div className="grid min-h-0 gap-3 lg:grid-cols-[1.08fr_.92fr_1fr]">
      ...
      <Section title="Aktivitas Terbaru" description="Perubahan terakhir pada konteks aktif." href="/riwayat" icon={<Clock3 size={14} />}><div className="space-y-2.5">{activity.slice(0, 4).map((a) => { const isGuru = a.entityType.toLowerCase().replace(/[- ]+/g, "_") === "guru"; const guru = isGuru && a.entityLabel ? guruByName.get(a.entityLabel) : undefined; return <Link key={a.id} href="/riwayat" className="group flex items-start gap-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/40">{isGuru ? <Avatar name={a.entityLabel} size="sm" kodeGuru={guru?.kodeGuru} jenisKelamin={guru?.jenisKelamin} /> : <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-brand-600/70" />}<div className="min-w-0"><p className="truncate text-[10px] font-medium text-ink-800 group-hover:text-brand-700">{a.action}</p><time className="text-[8.5px] text-ink-400">{new Date(a.createdAt).toLocaleString("id-ID", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit", timeZone: "Asia/Jakarta" })}</time></div></Link>; })}</div></Section>
    </div>
