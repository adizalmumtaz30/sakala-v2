...
function AgendaList({ agenda, guruList }: { agenda: DashboardAgendaEntry[]; guruList: GuruLite[] }) {
  return <div className="space-y-1">{agenda.slice(0, 4).map((e) => { const teacher = e.teacher?.trim() || "Guru"; const className = e.className?.trim() || "Kelas"; const g = e.teacherId ? guruList.find((item) => item.id === e.teacherId) : undefined; return <Link key={e.id} href={`/jadwal?assignment=${encodeURIComponent(e.id)}`} aria-label={`${e.subject} · ${teacher} · ${className}`} className={`group flex items-center gap-2 rounded-[11px] px-1.5 py-2 hover:bg-surface-muted ${focusRing}`}><span className="w-[42px] shrink-0 text-[8.5px] font-bold tabular-nums text-brand-600">{e.time}</span><Avatar name={g?.namaGuru ?? teacher} size="sm" /><span className="min-w-0 flex-1"><span className="block truncate text-[9.5px] font-semibold text-ink-900 group-hover:text-brand-700">{e.subject?.trim() || "Mata pelajaran"}</span><span className="block truncate text-[7.8px] text-ink-400">{className}{e.room ? ` · ${e.room}` : ""}</span></span></Link>; })}</div>;
}
  ...
  return <>
    <main className="mx-auto flex min-h-[calc(100vh-5.5rem)] max-w-[1480px] flex-col gap-4 px-3 pb-24 pt-4 sm:px-5 lg:gap-5 lg:px-6">
      ...
      <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_315px] xl:gap-5">
        ...
        <aside className="min-w-0 space-y-4 lg:space-y-5" aria-label="Informasi pendukung Dashboard"><Section title="Kalender" description="Navigasi cepat ke jadwal." icon={<CalendarDays size={14} />}><MiniCalendar /></Section><Section title="Agenda Hari Ini" description="Jadwal terdekat dari konteks aktif." href="/jadwal" icon={<Clock3 size={14} />}><AgendaList agenda={agenda} guruList={guruList} /></Section><Section title="Notifikasi Terbaru" description="Sinyal dan perubahan terakhir." href="/notifikasi" icon={<Bell size={14} />}><ActivityList activity={activity} /></Section></aside>
      </div>
